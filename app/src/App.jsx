...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] to-[#1a1f2b] p-4 font-[Inter]">
      ...
      <p className="text-center text-sm text-gray-400 mb-4">{`Całkowity P&L: $${totalPnl}`}</p>
      <div className="max-w-md mx-auto p-6 space-y-4 bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl">
        ...
        {message && (
          <div className="fixed top-4 right-4 bg-white/20 text-white px-4 py-2 rounded-lg shadow-lg fade-in show">
            {message}
          </div>
        )}
      </div>
          ...
          <div key={e.id} className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/20">
              ...
              <div className="flex space-x-6 items-center">
                ...
                <button className="text-red-400 hover:text-red-300" onClick={() => deleteEntry(e.id)}>
                  &times;
                </button>
              </div>
            </div>
